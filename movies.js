...
window.addEventListener('DOMContentLoaded', async function(event) {
    ...
    let moviesList = querySnapshot.docs
    for (let i = 0; i < json.results.length; i++) {
        console.log(json.results[i])
        let movieID = json.results[i].id
        let movieTitle = json.results[i].original_title
        let moviePosterName = json.results[i].poster_path
        let moviePoster = `https://image.tmdb.org/t/p/w500/${moviePosterName}`
        ...
        <img src="https://image.tmdb.org/t/p/w500/${moviePosterName}" class="w-full">
        <a href="#" class="watched-button block text-center text-white bg-green-500 mt-4 px-4 py-2 rounded">I've watched this!</a>
      </div>
        `)
    }
